...
| `canvas.addEventListener("mousemove"/"mousedown", (e) => { const x = e.clientX - rect.left; ... })`                   | Delete the coordinate math. Set `pointer: true` on the `GameRuntime` entry and override `handlePointerMove(x, y)`/`handlePointerDown(x, y)` on your engine — `GameCanvas` translates client coordinates to your canvas's logical space and calls them directly, the same way it calls `handleKeyDown`/`handleKeyUp`. |
